        ...
        if (user != null) {
            ...
            db.collection('donor').insertOne(data, (err, collection) => {
                ...
                db.collection("transaction").find().sort({"Timestamp":-1}).limit(6).toArray(function (err, result) {
                    if (err) throw err;
                   return res.render('home-donor', {user:{Name:name, MobileNo:phone, Email:mail}, trans: result });
                })
            });
        }
    });
    ...
    db.collection('ben').findOne({ $and: [{ Name: name }, { Adhaar: adhaar }, { Email: mail }, { MobileNo: phone }] }, function (err, user) {
        if (user != null) {
            ...
            db.collection('ben').insertOne(data, (err, collection) => {
                ...
                db.collection("transaction").find().sort({"Timestamp":-1}).limit(6).toArray(function (err, result) {
                    if (err) throw err;
                   return res.render('home-ben', { user:{Name:name, MobileNo:phone, Email:mail},trans: result });
                })
            });
        }
    });
...
app.post("/login-ben", (req, res) => {
    ...
    db.collection('ben').findOne({ Email: mail, Password: password }, function (err, user) {
        ...
        db.collection("transaction").find().sort({"Timestamp":-1}).limit(6).toArray(function (err, result) {
            if (err) throw err;
           return res.render('home-ben', {user:user, trans: result });
        })
    });
})
app.post("/login-donor", (req, res) => {
    ...
    db.collection('donor').findOne({ Email: mail, Password: password }, function (err, user) {
        ...
        db.collection("transaction").find().sort({"Timestamp":-1}).limit(6).toArray(function (err, result) {
            if (err) throw err;
           return res.render('home-donor', {user:user, trans: result });
        })
    });
})
app.get('/ben', (req, res) => {
...
app.post("/org", (req, res) => {
    ...
    db.collection('org').findOne({ $and: [{ Bid: bid }, { Name: name }, { Id: id }, { MobileNo: phone }] }, function (err, user) {
        console.log(bid);
        if (user != null) {
...
app.get('/org_details', (req, res) => {
    const u = Object.keys(req.query)
    const uid = u[0];
    console.log('donate id ', uid);
    db.collection("org").findOne({ "_id": new ObjectID(uid) }, function (err, result) {
        if (err) throw err;
        console.log(result);
        return res.render('org_details', { proj: result })
    })
})
                                    ...
                                    db.collection("projects_to_donate").findOne({"_id": new ObjectID(req.query.id)}, function (err,user){
                                        if(user.RequiredAmount==0)
                                        {
                                            ...
                                            var data = {
                                                ...
                                                "Amount": user.Amount,
                                                "Adhaar": user.Adhaar,
                                                "ProjectHolder":user.ProjectHolder,
                                                "Link":user.Link,
                                                "Bid":user.Bid
                                            }
                                            db.collection('completed_projects').insertOne(data, (err, collection) => {
                                                if (err) {
                                                    throw err;
                                                }
                                                else{
                                                console.log("REcord inserted successfully");
                                                db.collection("projects_to_donate").deleteOne({ "_id": new ObjectID(req.query.id) }, function (err, result) {
                                                    if (err) throw err;
                                                    console.log(req.query.id + 'deleted');
...
app.get('/completed_proj', (req, res) => {
    db.collection("completed_projects").find().sort().toArray(function (err, result) {
        if (err) throw err;
        res.render('completed_proj', { projs: result });
    })
});
app.get('/org-see', (req, res) => {
    db.collection("org").find().sort().toArray(function (err, result) {
        if (err) throw err;
        res.render('org_see', { orgs: result });
    })
});
...
app.get('/add-projects-req', (req, res) => {
    ...
    if (req.query.uid) {
        db.collection('ben').findOne({ "_id": new ObjectID(req.query.uid) }, function (err, user) {
            console.log(user.Adhaar);
           return res.render('add_projects_req',{info: user});

        });
    }
    //return res.redirect('login-ben');
});
